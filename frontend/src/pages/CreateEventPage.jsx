...
export default function CreateEventPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/host/invitation', { state: { eventId: event.id } });
    } catch (err) {
      setError(err.message || 'Could not create event. Please check your details and try again.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <main className="main-content">
        <div className="create-event-outer">
          ...
          <form className="create-event-form card" onSubmit={handleSubmit}>
            ...
            <button type="submit" className="btn-primary create-event-submit" disabled={submitting}>
              {submitting ? 'Creating Event...' : 'Create Event'}
            </button>
            <p style={{ textAlign: 'center', fontSize: '11px', color: 'var(--text-light)', marginTop: 8 }}>
              By creating an event, you agree to our <a href="#" style={{ color: 'var(--primary)' }}>Terms of Service</a>
            </p>
          </form>
        </div>
      </main>
...
function ChurchIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/></svg>; }
//i just added this so hopeully the vercel redeploy now 