...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    if (token && userData) {
      ...
      // Keep api/client.js (used by most feature pages) in sync with this token
      setClientAccessToken(token);
    }
    setLoading(false);

    // If api/client.js's silent refresh ever fails, log the user out everywhere
  ...
  const login = async ({ email, password }) => {
    ...
    try {
      ...
      // Keep api/client.js (used by HostDashboard, ProfilePage, CreateEventPage, etc.) in sync
  ...
  const googleLogin = async (idToken) => {
    setError(null);
    try {
      const response = await axios.post(`${API_URL}/auth/google`, { idToken }, { withCredentials: true });
      const { accessToken, user } = response.data.data;
